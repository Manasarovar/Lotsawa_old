...
from GVector_dict import dictReport
...
class GVector:
	...
	def push_back(self,strData):
		...
		if(self.recordArraySize<self.recordCount+1):
			...
			if(self.recordArraySize*INDEX_RESIZE*8+self.dataSize>self.poolSize):
...
class dictBase:
	def __init__(self):
		...
		self.history=list()
		self.vData=GVector()
		self.pIndex=''  #path to index file
		self.dictKey={}
		self.dictSize=0
		self.fileSize=0
		self.step=0
	...
	def getsub(self,subkey):
		...
		if(len(self.keyList)==0):
			print ('no data')
			return self.res
		pattern=f'(\S*{subkey}\S*)'
		sent_lst=list(set(re.findall(pattern, str(self.keyList))))
	...
	def saveInd(self):
		with open(self.pIndex, 'w', newline="\n", encoding='utf-8') as f:
			f.writelines(self.keyList)
			#print 'done save'
		self.insert=0
		self.step=0
	...
	def loadTXT(self,fileName,mode):
		print ('load {} mode {}'.format(fileName,mode))
